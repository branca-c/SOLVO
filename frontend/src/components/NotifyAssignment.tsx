...
export function NotifyAssignment({ id, onSent, onBusy }: { id: number; onSent: () => void; onBusy?: (busy: boolean) => void }) {
  ...
  return <div className="assignment-notify">
    ...
    {result && <div role="status"><p>{result.provider === 'mock' ? 'Invio simulato: nessun messaggio WhatsApp inviato.' : 'Notifica inoltrata a Twilio. Consegna non ancora verificata.'}</p><a href={result.action_url} target="_blank" rel="noopener noreferrer">Apri link tecnico</a></div>}
  </div>
}
